...
st.markdown("First, select datasheet in side bar")
...
lsi = pd.read_csv(
    r"C:\Users\sugan\Box\IIIS\metab\21年ストレス試験MM\0_data\LSI_blood\input\210708LSIRawdata.csv"
)
...
for res, male, female in zip(
    stress_res_list,
    stress_res_male_item_list,
    stress_res_female_item_list,
):
...
scr["CELLNAME"] = pd.Categorical(
    scr.CELLNAME, ordered=True, categories=["ストレス低", "ストレス中", "ストレス高"]
...
def summary_LSI_single_by_stress(data: pd.DataFrame, lsi_value_item=None):
    if lsi_value_item is None:
        return
...
column_ = st.sidebar.selectbox(
    "Sort column", ["Age", "Sex", "Married", "Child", "Jobtype", "Hincome", "CELLNAME"]
)
...
def lsi_data_by_stress(data):
    ...
    summary = summary_LSI_single_by_stress(data, lsi_value_item)
    if summary is None:
